...
function getKeyDetailsString (key, res, next) {
  let redisConnection = res.locals.connection;
  redisConnection.getBuffer(key, function (err, val) {
    if (err) {
    ...
    let details = {
      key: key,
      type: 'string',
      value: val.toString('utf8'),
    };
    const bytes = new Uint8Array(val); //val instanceof ArrayBuffer ? val : val.buffer);
    const decoder = new TextDecoder("ascii");

    const isAudio = decoder.decode(bytes.slice(0, 4)) === "RIFF" &&  decoder.decode(bytes.slice(8, 12)) === "WAVE";
    if (isAudio) {
      details.type = 'audio';
      details.value = Buffer.from(val).toString('base64');
    }
    // check if binary data / contains control chars
    else if (config.get('ui.binaryAsHex') && ControlCharRegEx.test(val)) {
      details.type = 'binary';
      details.value = Buffer.from(val).toString('base64');
    }
  ...
  redisConnection.type(key, function (err, type) {
    ...
    myutil.decodeHTMLEntities(req.body.stringValue, function (value) {
      ...
      switch (type) {
          ...
          case 'audio':
            return editAudio(key, value, req, res, next);
          case 'list':
...
function editAudio (key, value, req, res, next) {
  if (!req.app.locals.noLogData) console.log('new value for key: ', value);
  let redisConnection = res.locals.connection;
  const audioBuffer = Buffer.from(value, 'base64');
  redisConnection.set(key, audioBuffer, function (err) {
    if (err) {
      console.error('editAudio', err);
      return next(err);
    }
    res.send('OK');
  });
